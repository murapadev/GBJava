...
import gbc.controller.CpuSnapshot;
...
public class DebugView extends JFrame {
    ...
    private void initializeComponents() {
        ...
        stepButton = createToolbarButton("Step", IconFactory.step(), "Execute one CPU instruction (F7)");
        runButton = createToolbarButton("Run", IconFactory.play(), "Resume continuous execution (F5)");
        pauseButton = createToolbarButton("Pause", IconFactory.pause(), "Pause execution (F6)");
        resetButton = createToolbarButton("Reset", IconFactory.reset(), "Reset emulator state (Ctrl+R)");
        jumpToPcButton = createToolbarButton("Jump to PC", IconFactory.target(),
                "Center views around the current program counter");
    ...
    private void stepExecution() {
        ...
        updateAllViews();
        CpuSnapshot snapshot = snapshot();
        if (snapshot != null) {
            statusLabel.setText("Stepped - PC: $" + Integer.toHexString(snapshot.pc()).toUpperCase());
        }
    }
    ...
    private void resetEmulator() {
        ...
        CpuSnapshot snapshot = snapshot();
        if (snapshot != null) {
            statusLabel.setText("Reset - PC: $" + Integer.toHexString(snapshot.pc()).toUpperCase());
        }
    }
    ...
    public void setController(EmulatorActions controller) {
        ...
        if (controller == null) {
            ...
            reflectPauseState(controller.isPaused());
        }
    }
    ...
    private void updateStatusIndicators() {
        ...
        int pc = snapshot.pc();
        int sp = snapshot.sp();
        boolean ime = snapshot.ime();
        boolean halted = snapshot.halted();
        boolean doubleSpeed = snapshot.doubleSpeed();
    ...
    private class MemoryTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int column) {
            ...
            if (column == 0) {
                ...
                int value = readByte(byteAddress);
    ...
    private class DisassemblerPanel extends JPanel {
        ...
        public void refresh() {
            ...
            CpuSnapshot snapshot = snapshot();
            if (snapshot == null) {
                disassemblyArea.setText("Controller unavailable.");
                return;
            }
            ...
            while (bytesDecoded < maxBytes) {
                DecodedInstruction decoded = decodeInstruction(addr);
                if (decoded == null) {
                    break;
                }
        ...
        private boolean isCartridgeLoaded() {
            return DebugView.this.isCartridgeLoaded();
        }
    ...
    private class RegisterPanel extends JPanel {
        ...
        public void refresh() {
            ...
            CpuSnapshot snapshot = snapshot();
            if (snapshot == null) {
                registerArea.setText("Controller unavailable.");
                return;
            }
            StringBuilder sb = new StringBuilder();
        ...
        private void searchTextValue(String text, StringBuilder results) {
            ...
            for (int addr = 0x0000; addr <= 0xFFFF - searchBytes.length; addr++) {
                ...
                for (int i = 0; i < searchBytes.length; i++) {
                    if (readByte(addr + i) != (searchBytes[i] & 0xFF)) {
                        match = false;
                        break;
                    }
                }
    ...
    private boolean isCartridgeLoaded() {
        return diagnostics != null && diagnostics.isCartridgeLoaded();
    }

    private CpuSnapshot snapshot() {
        return diagnostics != null ? diagnostics.cpuSnapshot() : null;
    }

    private int readByte(int address) {
        return diagnostics != null ? diagnostics.readByte(address) : 0;
    }

    private DecodedInstruction decodeInstruction(int address) {
        return diagnostics != null ? diagnostics.decodeInstruction(address) : null;
    }
}
