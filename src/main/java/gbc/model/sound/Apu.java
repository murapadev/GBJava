...
public class Apu {
    ...
    public Apu() {
        ...
        int bufferSize = Integer.getInteger("audio.bufferSize", 1024);
    ...
    private void stepInternal() {
        ...
        // Generate sample based on configured sample rate
        if (cycleCounter >= cyclesPerSample) {
            ...
            if (bufferPosition == internalBuffer.length) {
                ...
                if (AUDIO_DEBUG) {
                    bufferFillCount++;
                    if ((bufferFillCount % 20) == 0) {
                        logDebug("[APU] buffers=" + bufferFillCount);
                    }
                }
            }
        }
