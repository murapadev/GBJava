...
public final class AudioEngine implements AutoCloseable {
    ...
    public AudioEngine() {
        ...
        Integer configuredBuffer = Integer.getInteger("audio.bufferSize", 0);
    ...
    private SourceDataLine createSourceLine(AudioFormat format) throws LineUnavailableException {
        if (AUDIO_MIXER == null || AUDIO_MIXER.isBlank()) {
            SourceDataLine preferred = selectPreferredMixer(format);
            if (preferred != null) {
                return preferred;
            }
        ...
        for (javax.sound.sampled.Mixer.Info info : AudioSystem.getMixerInfo()) {
            if (info.getName().toLowerCase().contains(AUDIO_MIXER.toLowerCase())
                ...
                return (SourceDataLine) mixer.getLine(new javax.sound.sampled.DataLine.Info(SourceDataLine.class, format));
            }
        }
        logDebug("Audio mixer not found: " + AUDIO_MIXER + " (falling back to default)", null);
        return AudioSystem.getSourceDataLine(format);
    }
    ...
    private SourceDataLine selectPreferredMixer(AudioFormat format) {
        javax.sound.sampled.Mixer.Info[] mixers = AudioSystem.getMixerInfo();
        javax.sound.sampled.DataLine.Info lineInfo = new javax.sound.sampled.DataLine.Info(SourceDataLine.class, format);
        javax.sound.sampled.Mixer.Info best = null;
        for (javax.sound.sampled.Mixer.Info info : mixers) {
            ...
            boolean isJava = name.contains("java") || desc.contains("java");
            boolean isDirect = name.contains("direct") || desc.contains("direct");
            if (isJava) {
                continue;
            }
            if (best == null) {
        ...
        if (best != null) {
            logDebug("Auto-selected mixer: " + best.getName(), null);
            javax.sound.sampled.Mixer mixer = AudioSystem.getMixer(best);
            try {
                return (SourceDataLine) mixer.getLine(lineInfo);
            } catch (LineUnavailableException e) {
                logDebug("Auto-selected mixer failed, falling back", e);
            }
        }
        return null;
    }
